...
#' residuals of the regression using least square error (Logit)
#'
...
logit_reg_loss=function(reg_dt,p){
  if(is.null(nrow(reg_dt))){return(rep(0,0))}else
  {
    if(nrow(reg_dt)<p){return(reg_dt[,1]-mean(reg_dt[,1]))}else
    {return(glm(y~.,data=reg_dt,family =  binomial(link = "logit"))$residuals)}}
}
